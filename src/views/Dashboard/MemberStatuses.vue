  ...
  <div class="master-data-page">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Status Jemaat</h1>
        <p class="page-subtitle">Kelola daftar status jemaat di cabang ini</p>
      </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              ...
              <p>{{ editingId ? 'Ubah informasi status jemaat yang sudah ada' : 'Masukkan nama status jemaat baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                <label class="form-label">Nama Status Jemaat</label>
                <div class="input-wrapper">
                  <Type size="18" class="input-icon" />
                  <input v-model="form.name" type="text" required placeholder="Contoh: Jemaat, Simpatisan, dll." :disabled="saving" />
                </div>
              </div>
            </div>
...
const fetchData = async (page = 1) => {
  ...
  loading.value = true;
  try {
    const response = await axios.get('/member-statuses', {
      params: { 
    ...
    Swal.fire('Error', 'Gagal memuat data status jemaat.', 'error');
  } finally {
    loading.value = false;
  }
};
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/member-statuses/${editingId.value}`, form.value);
    } else {
      await axios.post('/member-statuses', form.value);
    }
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data status jemaat telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/member-statuses/${item.id}`);
      await fetchData(pagination.value.current_page);
