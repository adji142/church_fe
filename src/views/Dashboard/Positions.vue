  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        ...
        <p class="page-subtitle">Kelola daftar jabatan dan struktur pelaporan di cabang ini</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()" :disabled="saving">
          <Plus size="18" /> <span>Tambah Jabatan</span>
        </button>
      </div>
    </div>
        ...
        <table class="modern-table">
          ...
          <tbody>
            <tr v-for="(item, index) in data" :key="item.id" class="row-hover">
              ...
              <td>
                <span v-if="item.department" class="dept-tag">{{ item.department.name }}</span>
                <span v-else class="text-muted text-xs">-</span>
              </td>
              <td>
                <span v-if="item.parent" class="parent-tag">{{ item.parent.name }}</span>
                <span v-else class="text-muted text-xs italic">Puncak (Top)</span>
              </td>
              <td>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Jabatan' : 'Tambah Jabatan Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi jabatan yang sudah ada' : 'Sebutkan nama jabatan dan hierarkinya' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: Gembala, Bendahara, Koordinator, dll." :disabled="saving" />
                </div>
              </div>
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <select v-model="form.department_id" required :disabled="saving">
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <select v-model="form.parent_id" :disabled="saving">
                    <option :value="null">-- Tidak Ada (Pimpinan Tertinggi) --</option>
                    <option v-for="pos in filteredPositions" :key="pos.id" :value="pos.id" :disabled="pos.id === editingId">
                      {{ pos.name }} ({{ pos.department?.name || 'No Dept' }})
                    </option>
                  </select>
                </div>
              </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Jabatan' : 'Simpan Jabatan' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Plus, Edit2, Trash2, X, Search, Check, Type, ChevronLeft, ChevronRight, Bookmark, Briefcase
...
const fetchData = async (page = 1) => {
  ...
  loading.value = true;
  try {
    const [paginatedRes, deptRes, posRes] = await Promise.all([
      axios.get('/positions', {
        params: { 
      ...
      axios.get('/departments', { params: { branch_id: auth.activeBranch.id, per_page: 999 } }),
      axios.get('/positions', { params: { branch_id: auth.activeBranch.id, per_page: 999 } })
    ]);

    data.value = paginatedRes.data.data;
    pagination.value = {
      current_page: paginatedRes.data.current_page,
      last_page: paginatedRes.data.last_page,
      per_page: paginatedRes.data.per_page,
      total: paginatedRes.data.total,
      from: paginatedRes.data.from,
      to: paginatedRes.data.to
    };
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/positions/${editingId.value}`, form.value);
    } else {
      await axios.post('/positions', form.value);
    }
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Jabatan "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/positions/${item.id}`);
      await fetchData(pagination.value.current_page);
