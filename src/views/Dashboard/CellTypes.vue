  ...
  <div class="master-data-page">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Jenis Cell</h1>
        <p class="page-subtitle">Kelola kategori jenis cell (contoh: Youth, Kids, General)</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()" :disabled="saving">
          <Plus size="18" /> <span>Tambah Jenis</span>
        </button>
      </div>
    </div>
        ...
        <table class="modern-table">
          ...
          <tbody>
            <tr v-for="(item, index) in data" :key="item.id" class="row-hover">
              ...
              <td>
                <span v-if="item.start_age || item.end_age" class="age-badge">
                  {{ item.start_age || 0 }} - {{ item.end_age || '∞' }} Tahun
                </span>
                <span v-else class="text-muted">Semua Usia</span>
              </td>
              <td>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Jenis Cell' : 'Tambah Jenis Cell Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi jenis cell yang sudah ada' : 'Masukkan informasi kategori jenis cell baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                <label class="form-label">Nama Jenis Cell</label>
                <div class="input-wrapper">
                  <Type size="18" class="input-icon" />
                  <input v-model="form.name" type="text" required placeholder="Contoh: Youth, Professional, Kids" :disabled="saving" />
                </div>
              </div>
              ...
              <div class="flex gap-4 mt-4">
                <div class="form-group flex-1">
                  ...
                  <div class="input-wrapper">
                    <input v-model.number="form.start_age" type="number" placeholder="0" :disabled="saving" />
                  </div>
                </div>
                <div class="form-group flex-1">
                  <label class="form-label">Usia Selesai</label>
                  <div class="input-wrapper">
                    <input v-model.number="form.end_age" type="number" placeholder="99" :disabled="saving" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Jenis' : 'Simpan Jenis' }}</span>
                </template>
              </button>
            </div>
          </form>
...
const fetchData = async (page = 1) => {
  ...
  loading.value = true;
  try {
    const response = await axios.get('/cell-types', {
      params: { 
    ...
    console.error('Failed to fetch cell types', error);
    Swal.fire('Error', 'Gagal memuat data jenis cell.', 'error');
  } finally {
    loading.value = false;
  }
};
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/cell-types/${editingId.value}`, form.value);
    } else {
      await axios.post('/cell-types', form.value);
    }
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data jenis cell telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Jenis cell "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/cell-types/${item.id}`);
      await fetchData(pagination.value.current_page);
