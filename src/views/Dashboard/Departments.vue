  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Departemen</h1>
        <p class="page-subtitle">Kelola struktur departemen organisasi di cabang ini</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()" :disabled="saving">
          <Plus size="18" /> <span>Tambah Departemen</span>
        </button>
      </div>
    </div>
          ...
          <tbody>
            <tr v-for="(item, index) in data" :key="item.id" class="row-hover">
              ...
              <td>
                ...
                <span v-else class="text-muted text-xs italic">Akar (Root)</span>
              </td>
              <td>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Departemen' : 'Tambah Departemen Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi departemen yang sudah ada' : 'Masukkan nama departemen baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: Departemen Musik, Pemuda, dll." :disabled="saving" />
                </div>
              </div>

              <div class="form-group full-width">
                <label class="form-label">Departemen Induk (Opsional)</label>
                <div class="input-wrapper">
                  <Layers size="18" class="input-icon" />
                  <select v-model="form.parent_id" :disabled="saving">
                    <option :value="null">-- Tidak Ada (Sebagai Akar) --</option>
                    <option v-for="dept in allDepartments" :key="dept.id" :value="dept.id" :disabled="dept.id === editingId">
                      {{ dept.name }}
                    </option>
                  </select>
                </div>
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  <textarea v-model="form.description" rows="3" placeholder="Informasi tambahan..." :disabled="saving" class="modern-textarea"></textarea>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Departemen' : 'Simpan Departemen' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Plus, Edit2, Trash2, X, Search, Check, Type, ChevronLeft, ChevronRight, Layers
...
const fetchData = async (page = 1) => {
  ...
  try {
    ...
    allDepartments.value = allRes.data.data;
  } catch (error) {
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/departments/${editingId.value}`, form.value);
    } else {
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data departemen telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Departemen "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
...
.input-wrapper input, .input-wrapper select, .modern-textarea {
  width: 100%;
...
.input-wrapper input:focus, .input-wrapper select:focus, .modern-textarea:focus {
  outline: none;
  border-color: #6366f1;
  background: white;
