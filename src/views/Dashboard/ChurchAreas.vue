  ...
  <div class="master-data-page">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Wilayah Jemaat</h1>
        <p class="page-subtitle">Kelola daftar wilayah jemaat di cabang ini</p>
      </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Wilayah' : 'Tambah Wilayah Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi wilayah yang sudah ada' : 'Masukkan nama wilayah baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: Wilayah I, Wilayah II, dll." :disabled="saving" />
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Wilayah' : 'Simpan Wilayah' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Plus, Edit2, Trash2, X, Search, Check, MapPin, ChevronLeft, ChevronRight
...
const fetchData = async (page = 1) => {
  ...
  loading.value = true;
  try {
    const response = await axios.get('/church-areas', {
      params: { 
    ...
    console.error('Failed to fetch church areas', error);
    Swal.fire('Error', 'Gagal memuat data wilayah jemaat.', 'error');
  } finally {
    loading.value = false;
  }
};
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/church-areas/${editingId.value}`, form.value);
    } else {
      await axios.post('/church-areas', form.value);
    }
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data wilayah jemaat telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Wilayah "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/church-areas/${item.id}`);
      await fetchData(pagination.value.current_page);
