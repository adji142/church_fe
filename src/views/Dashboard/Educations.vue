  ...
  <div class="master-data-page">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Pendidikan</h1>
        <p class="page-subtitle">Kelola daftar jenjang pendidikan di cabang ini</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()" :disabled="saving">
          <Plus size="18" /> <span>Tambah Pendidikan</span>
        </button>
      </div>
    </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Pendidikan' : 'Tambah Pendidikan Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi jenjang pendidikan yang sudah ada' : 'Masukkan nama jenjang pendidikan baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: SD, SMP, SMA, S1, dll." :disabled="saving" />
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Pendidikan' : 'Simpan Pendidikan' }}</span>
                </template>
              </button>
            </div>
          </form>
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/educations/${editingId.value}`, form.value);
    } else {
      await axios.post('/educations', form.value);
    }
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data pendidikan telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Pendidikan "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/educations/${item.id}`);
      await fetchData(pagination.value.current_page);
