  ...
  <div class="master-data-page">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Master Status Pernikahan</h1>
        <p class="page-subtitle">Kelola daftar status pernikahan untuk jemaat di cabang ini</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()" :disabled="saving">
          <Plus size="18" /> <span>Tambah Status</span>
        </button>
      </div>
    </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Status' : 'Tambah Status Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi status pernikahan yang sudah ada' : 'Masukkan nama status pernikahan baru' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-section">
              <div class="form-group full-width">
                <label class="form-label">Nama Status Pernikahan</label>
                <div class="input-wrapper">
                  <Type size="18" class="input-icon" />
                  <input v-model="form.name" type="text" required placeholder="Contoh: Belum Menikah, Menikah, dll." :disabled="saving" />
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary btn-lg" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Status' : 'Simpan Status' }}</span>
                </template>
              </button>
            </div>
          </form>
...
const fetchData = async (page = 1) => {
  ...
  try {
    ...
    Swal.fire('Error', 'Gagal memuat data status pernikahan.', 'error');
  } finally {
    loading.value = false;
  }
};
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/marital-statuses/${editingId.value}`, form.value);
    } else {
      await axios.post('/marital-statuses', form.value);
    }
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data status pernikahan telah diperbarui.',
      timer: 2000,
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Status "${item.name}" akan dihapus permanen!`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/marital-statuses/${item.id}`);
      await fetchData(pagination.value.current_page);
