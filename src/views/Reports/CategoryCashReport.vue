  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Laporan Kas per Kategori</h1>
        <p class="page-subtitle">Analisis alokasi penggunaan dana berdasarkan kategori transaksi</p>
      </div>
    ...
    <div class="card filter-card report-section-gap">
      ...
      <div class="p-6">
        <div class="filter-item min-w-[200px]">
            <span class="filter-label">Jenis Kas</span>
    ...
    <div class="card table-card shadow-sm border-slate-200">
      ...
      <div v-else-if="reportData.length === 0" class="empty-state h-64">
        ...
        <p>Tidak ada data ditemukan untuk kriteria ini.</p>
      </div>
      ...
      <div v-else class="table-container shadow-none">
        <table class="modern-table border-none">
          <thead class="bg-slate-50/50">
            <tr>
              <th>Kategori Kas</th>
              <th class="text-right" style="width: 250px">Total Nominal</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in reportData" :key="index" class="row-hover border-b border-slate-100 last:border-0">
              ...
              <td class="text-right font-black" :class="filters.type === 'income' ? 'text-emerald-600' : 'text-rose-600'">
          ...
          <tfoot class="bg-slate-50/30">
            <tr>
              <td class="font-black text-slate-500 uppercase text-[10px] py-6">TOTAL KESELURUHAN</td>
              <td class="text-right font-black text-xl" :class="filters.type === 'income' ? 'text-emerald-600' : 'text-rose-600'">
...
import { 
  Calendar, MapPin, ArrowUpDown, RefreshCcw, FileText
...
const totalSum = computed(() => {
    return reportData.value.reduce((sum, item) => sum + parseFloat(item.total_nominal || 0), 0);
});

const fetchMasterData = async () => {
  try {
    const branchRes = await axios.get('/branches', { params: { per_page: 999 } });
...
const fetchReport = async () => {
  loading.value = true;
  try {
    const res = await axios.get('/reports/cash-category', {
