  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Dana Gereja</h1>
        <p class="page-subtitle">Kelola alokasi dana dan sumber pendanaan pelayanan</p>
      </div>
      <div class="header-actions">
        <div class="search-wrapper">
          ...
          <input 
            v-model="searchQuery" 
            type="text" 
            placeholder="Cari dana..." 
            class="search-input"
            @input="debounceSearch"
          />
        </div>
        <button class="btn btn-primary" @click="openModal()">
          <Plus size="18" /> <span>Tambah Dana</span>
        </button>
      </div>
    </div>
    ...
    <div class="card table-card">
      <div v-if="loading" class="loading-state">
        ...
        <p>Memuat data dana...</p>
      </div>
      <div v-else-if="data.length === 0" class="empty-state">
        <div class="empty-icon"><Briefcase size="48" /></div>
        <p>Tidak ada dana ditemukan.</p>
      </div>
      <div v-else class="table-container">
        <table class="modern-table">
          <thead>
            <tr>
              <th style="width: 250px">Nama Dana</th>
              <th>Kode</th>
              <th>Status</th>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Dana' : 'Tambah Dana Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi dana yang sudah ada' : 'Lengkapi informasi dana gereja di bawah ini' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-grid">
              <div class="form-group full-width">
                <label class="form-label">Nama Dana <span class="required">*</span></label>
                <div class="input-wrapper">
                  <Briefcase size="18" class="input-icon" />
                  <input v-model="form.name" type="text" required placeholder="Contoh: Dana Pembangunan" :disabled="saving" />
                </div>
              </div>
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  <textarea v-model="form.description" rows="3" placeholder="Informasi tambahan mengenai dana ini..." :disabled="saving"></textarea>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Data' : 'Simpan Dana' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Search, Plus, Edit2, Trash2, X, Check, ChevronLeft, ChevronRight, Briefcase
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/funds/${editingId.value}`, form.value);
    } else {
      await axios.post('/funds', form.value);
    }
...
const confirmDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/funds/${item.id}`);
      fetchData();
      Swal.fire('Terhapus!', 'Dana telah dihapus.', 'success');
    } catch (error) {
