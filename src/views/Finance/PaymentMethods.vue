  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        ...
        <p class="page-subtitle">Kelola opsi transaksi pembayaran donasi dan persembahan</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()">
          <Plus size="18" /> <span>Tambah Metode</span>
        </button>
      </div>
    </div>
    ...
    <div class="card table-card">
      <div v-if="loading" class="loading-state">
        ...
        <p>Memuat data metode...</p>
      </div>
      <div v-else-if="data.length === 0" class="empty-state">
        <div class="empty-icon"><CreditCard size="48" /></div>
        <p>Tidak ada metode ditemukan.</p>
      </div>
      <div v-else class="table-container">
        <table class="modern-table">
          <thead>
            <tr>
              <th style="width: 250px">Nama Metode</th>
              <th>Kode</th>
              <th>Status</th>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Metode' : 'Tambah Metode Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi metode pembayaran' : 'Pilih dan atur cara transaksi finansial di bawah ini' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-grid">
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: Transfer Bank BCA" :disabled="saving" />
                </div>
              </div>

              <div class="form-group full-width">
                <label class="form-label">Status Metode</label>
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  <textarea v-model="form.description" rows="3" placeholder="Informasi tambahan mengenai metode ini..." :disabled="saving"></textarea>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Data' : 'Simpan Metode' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Search, Plus, Edit2, Trash2, X, Check, ChevronLeft, ChevronRight, CreditCard
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/payment-methods/${editingId.value}`, form.value);
    } else {
...
const confirmDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/payment-methods/${item.id}`);
      fetchData();
      Swal.fire('Terhapus!', 'Metode telah dihapus.', 'success');
    } catch (error) {
