  ...
  <div class="master-data-page wide">
    <div class="header-section">
      <div class="title-group">
        <h1 class="page-title">Kategori Kas</h1>
        <p class="page-subtitle">Kelola klasifikasi pencatatan pemasukan dan pengeluaran</p>
      </div>
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openModal()">
          <Plus size="18" /> <span>Tambah Kategori</span>
        </button>
      </div>
    </div>
    ...
    <div class="card table-card">
      ...
      <div v-else-if="data.length === 0" class="empty-state">
        <div class="empty-icon"><Tag size="48" /></div>
      ...
      <div v-else class="table-container">
        <table class="modern-table">
          <thead>
            <tr>
              <th style="width: 250px">Identitas Kategori</th>
              <th>Tipe Kas</th>
              <th>Deskripsi</th>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="header-text">
              <h2>{{ editingId ? 'Update Kategori' : 'Tambah Kategori Baru' }}</h2>
              <p>{{ editingId ? 'Ubah informasi kategori yang sudah ada' : 'Lengkapi informasi kategori kas di bawah ini' }}</p>
            </div>
          ...
          <form @submit.prevent="saveData" class="modal-body">
            <div class="form-grid">
              <div class="form-group">
                ...
                <div class="input-wrapper">
                  ...
                  <input v-model="form.name" type="text" required placeholder="Contoh: Kolekte Umum" :disabled="saving" />
                </div>
              </div>

              <div class="form-group">
                <label class="form-label">Tipe Kas <span class="required">*</span></label>
                <div class="input-wrapper">
                  <ArrowUpDown size="18" class="input-icon" />
                  <select v-model="form.type" required :disabled="saving">
              ...
              <div class="form-group full-width">
                ...
                <div class="input-wrapper">
                  <textarea v-model="form.description" rows="3" placeholder="Informasi tambahan mengenai kategori ini..." :disabled="saving"></textarea>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn btn-primary" :disabled="saving">
                ...
                <template v-else>
                  <Check size="18" />
                  <span>{{ editingId ? 'Update Data' : 'Simpan Kategori' }}</span>
                </template>
              </button>
            </div>
          </form>
...
import { 
  Search, Plus, Edit2, Trash2, X, Check, ChevronLeft, ChevronRight, Tag, ArrowUpDown
...
const fetchData = async (page = 1) => {
  loading.value = true;
  try {
    const res = await axios.get('/cash-categories', {
      params: { 
        page, 
...
const saveData = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/cash-categories/${editingId.value}`, form.value);
    } else {
      await axios.post('/cash-categories', form.value);
    }
...
const confirmDelete = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      await axios.delete(`/cash-categories/${item.id}`);
      fetchData();
